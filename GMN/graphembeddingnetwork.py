...
def graph_prop_once(node_states,
    ...
    if mask_from_idx is not None:
        messages = messages * mask_from_idx[:, None].clone()
...
class GraphPropLayer(nn.Module):
    ...
    def _compute_aggregated_messages(
        ...
        aggregated_messages = graph_prop_once(
            ...
            mask_from_idx=mask_from_idx if not isinstance(mask_from_idx, list) else mask_from_idx[0])
        ...
        if self._use_reverse_direction:
            reverse_aggregated_messages = graph_prop_once(
                ...
                aggregation_module=None,
                edge_features=edge_features,
                mask_from_idx=mask_from_idx if not isinstance(mask_from_idx, list) else mask_from_idx[1])
