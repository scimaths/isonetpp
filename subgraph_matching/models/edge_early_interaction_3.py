...
class EdgeEarlyInteraction3(torch.nn.Module):
    def __init__(
        self,
        max_node_set_size,
        max_edge_set_size,
        ...
        super(EdgeEarlyInteraction3, self).__init__()
    ...
    def forward(self, graphs, graph_sizes, graph_adj_matrices):
        ...
        edge_feature_store = torch.zeros(num_edges, self.message_dim * self.propagation_steps, device=self.device)
        ...
        for _ in range(self.time_update_steps):
            ...
            for prop_idx in range(1, self.propagation_steps + 1):
                ...
                updated_edge_feature_store[:, interaction_idx - self.message_dim: interaction_idx] = torch.clone(messages)
            ...
            edge_feature_store = interleaved_edge_features[padded_edge_indices]
