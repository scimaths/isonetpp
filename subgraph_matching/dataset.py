...
class SubgraphIsomorphismDataset:
    def __init__(self, mode, dataset_name, dataset_size, batch_size, data_type, dataset_base_path, experiment):
    ...
    def load_graphs(self, experiment):
        dataset_accessor = lambda file_name: os.path.join(
            self.dataset_base_path, f"{self.dataset_size}_dataset",
        ...
        query_graph_file = dataset_accessor(f"{mode_prefix}_{self.dataset_name}_{pair_count}_query_subgraphs.pkl")
    ...
    def _pack_batch(self, graphs):
        ...
        return GraphCollection(
            ...
            node_features = torch.ones(total_nodes, 1, dtype=torch.float, dtype=self.device),
            edge_features = torch.ones(total_edges, 1, dtype=torch.float, dtype=self.device)
        )
    ...
    def create_stratified_batches(self):
        ...
        num_pos_per_batch = np.ceil((pos_to_neg_ratio/(1 + pos_to_neg_ratio)) * self.batch_size)
        ...
        for idx in range(0, len(pos_pairs), num_pos_per_batch):
            batches_pos.append(pos_pairs[idx : idx + num_pos_per_batch])
            labels_pos.append([1.0] * num_pos_per_batch)
        for idx in range(0, len(neg_pairs), num_neg_per_batch):
            batches_neg.append(neg_pairs[idx : idx + num_neg_per_batch])
            labels_neg.append([0.0] * num_neg_per_batch)
