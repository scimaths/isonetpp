...
class CrossAttention(torch.nn.Module):
    def __init__(self, av, type_, dim, max_node_size):
    ...
    def forward(self, data,
        ...
        if self.type == 'lrl':
            ...
            dot_pdt_similarity = torch.sum(-self.relu1(partitions_1 - partitions_2), dim=2).reshape(batch_size, self.max_node_size, self.max_node_size)
        else:
...
class GMN_match_hinge_hinge_similarity(torch.nn.Module):
    ...
    def build_layers(self):
        ...
        self.max_node_size = max(self.av.MAX_QUERY_SUBGRAPH_SIZE,self.av.MAX_CORPUS_SUBGRAPH_SIZE)
        self.lrl_cross_attention_module = CrossAttention(self.av, 'hinge', prop_config['node_state_dim'], self.max_node_size)
