...
class GMN_match_hinge_replicated(torch.nn.Module):
    def __init__(self, av,config,input_dim):
        super(GMN_match_hinge_replicated, self).__init__()
        self.av = av
        self.device = 'cuda:0' if self.av.has_cuda and self.av.want_cuda else 'cpu'
        self.config = config
    ...
    def build_layers(self):
        ...
        self.vanilla_cross_attention_module = CrossAttention(self.av, None, prop_config['node_state_dim'], self.max_node_size)
    ...
    def forward(self, batch_data,batch_data_sizes,batch_adj):
        ...
        for i in range(self.config['graph_matching_net'] ['n_prop_layers']) :
            node_features_enc = self.prop_layer(node_features_enc, from_idx, to_idx,\
                                                ...
                                                cross_attention_module=self.vanilla_cross_attention_module)
...
class GMN_match_hinge_injective_attention(torch.nn.Module):
    def __init__(self, av,config,input_dim):
        super(GMN_match_hinge_injective_attention, self).__init__()
        self.av = av
        self.device = 'cuda:0' if self.av.has_cuda and self.av.want_cuda else 'cpu'
        self.config = config
        self.input_dim = input_dim
    ...
    def build_layers(self):
        ...
        self.max_node_size = max(self.av.MAX_QUERY_SUBGRAPH_SIZE,self.av.MAX_CORPUS_SUBGRAPH_SIZE)
        self.vanilla_cross_attention_module = CrossAttention(self.av, None, prop_config['node_state_dim'], self.max_node_size, injective_attention=True)
