...
class NodeEarlyInteractionAddingTest(torch.nn.Module):
    def __init__(self, av, config, input_dim):
        super(NodeEarlyInteractionAddingTest, self).__init__()
        self.av = av
        self.config = config
    ...
    def forward(self, batch_data, batch_data_sizes, batch_adj):
        ...
        num_new_nodes_batch_ = [ (batch_data_sizes[idx][1] - batch_data_sizes[idx][0]) for idx in range(batch_size) for q_node in range(batch_data_sizes[idx][0]) ]
        num_new_nodes_batch = [ 1 for idx in range(batch_size)]
        ...
        batch_data_sizes_flat_ = [sublist[1] for sublist in batch_data_sizes for _ in sublist]
        batch_data_sizes_flat = [sublist[1] if sub%2 == 1 else (sublist[0] + 1) for sublist in batch_data_sizes for sub in range(len(sublist))]
        batch_data_sizes_flat_tensor = torch.tensor(batch_data_sizes_flat, device=device, dtype=torch.long)
        cumulative_sizes = torch.cumsum(torch.tensor(self.max_set_size, dtype=torch.long, device=device).repeat(len(batch_data_sizes_flat_tensor)), dim=0)
        ...
        for time_idx in range(1, n_time_update_steps + 1):
            ...
            mult_factor[-sum(num_new_edges):] *= torch.tensor(num_new_nodes_batch_, device=device)
