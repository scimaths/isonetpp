...
class GMN_match_hinge_baseline(torch.nn.Module):
    def __init__(self, av,config,input_dim):
        super(GMN_match_hinge_baseline, self).__init__()
        self.av = av
        self.config = config
        self.input_dim = input_dim
        self.build_layers()
    ...
    def forward(self, batch_data,batch_data_sizes,batch_adj):
        ...
        for time_idx in range(n_time_update_steps):
            ...
            for i in range(n_prop_update_steps):
                if i == (n_prop_update_steps - 1):
                    node_features_enc, attention_past = self.prop_layer(node_features_enc, from_idx, to_idx,\
                                                    graph_idx,2*len(batch_data_sizes), \
                                                    self.similarity_func, edge_features_enc, 
                                                    batch_data_sizes_flat=batch_data_sizes_flat, 
                                                    max_node_size=self.max_node_size,
                                                    attention_past=attention_past,
...
class GMN_match_hinge_lrl_scoring_injective_attention_baseline(torch.nn.Module):
    def __init__(self, av,config,input_dim):
        super(GMN_match_hinge_lrl_scoring_injective_attention_baseline, self).__init__()
        self.av = av
