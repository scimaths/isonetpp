...
class GMN_match_hinge_lrl_injective_attention(torch.nn.Module):
    def __init__(self, av,config,input_dim):
        super(GMN_match_hinge_lrl_injective_attention, self).__init__()
        self.av = av
...
class GMN_match_hinge_lrl_scoring_injective_attention(torch.nn.Module):
    def __init__(self, av,config,input_dim):
        super(GMN_match_hinge_lrl_scoring_injective_attention, self).__init__()
        self.av = av
        self.device = 'cuda:0' if self.av.has_cuda and self.av.want_cuda else 'cpu'
        self.config = config
        self.input_dim = input_dim
...
class GMN_match_hinge_hinge_similarity_injective_attention(torch.nn.Module):
    def __init__(self, av,config,input_dim):
        super(GMN_match_hinge_hinge_similarity_injective_attention, self).__init__()
        self.av = av
        self.device = 'cuda:0' if self.av.has_cuda and self.av.want_cuda else 'cpu'
        self.config = config
        self.input_dim = input_dim
    ...
    def build_layers(self):
        ...
        self.max_node_size = max(self.av.MAX_QUERY_SUBGRAPH_SIZE,self.av.MAX_CORPUS_SUBGRAPH_SIZE)
        self.hinge_cross_attention_module = CrossAttention(self.av, 'hinge', prop_config['node_state_dim'], self.max_node_size, injective_attention=True)
...
class GMN_match_hinge_hinge_similarity_scoring_injective_attention(torch.nn.Module):
    def __init__(self, av,config,input_dim):
        super(GMN_match_hinge_hinge_similarity_scoring_injective_attention, self).__init__()
        self.av = av
        self.device = 'cuda:0' if self.av.has_cuda and self.av.want_cuda else 'cpu'
        self.config = config
        self.input_dim = input_dim
...
class GMN_match_hinge_lrl_scoring_injective_attention_inter(torch.nn.Module):
    def __init__(self, av,config,input_dim):
        super(GMN_match_hinge_lrl_scoring_injective_attention_inter, self).__init__()
        self.av = av
        self.device = 'cuda:0' if self.av.has_cuda and self.av.want_cuda else 'cpu'
        self.config = config
        self.input_dim = input_dim
    ...
    def build_layers(self):
        ...
        prop_config.pop('share_prop_params',None)
        prop_config.pop('similarity',None)        
        self.prop_layer = gmngmn.GraphPropMatchingLayerInter(**prop_config)      
