...
class EdgeEarlyInteractionDelete(torch.nn.Module):
    def __init__(self, av, config, input_dim):
        super(EdgeEarlyInteractionDelete, self).__init__()
        self.av = av
        self.config = config
    ...
    def build_layers(self):
        ...
        self.lambd = self.config["edge_early_interaction_delete"]["lambd"]
    ...
    def forward(self, batch_data, batch_data_sizes, batch_adj):
        ...
        for time_idx in range(1, n_time_update_steps + 1):
            ...
            for prop_idx in range(1, n_prop_update_steps + 1) :
                ...
                node_features_enc = self.prop_layer(node_features_enc, from_idx, to_idx, combined_features, mask_from_idx=delete_factor)
            ...
            # -------------------------------- CALCULATING DELETE FACTOR --------------------------------------------
            # (batch_size, MAX_EDGE, MAX_EDGE) -> (batch_size, MAX_EDGE)
            layout_transport_plan = torch.sum(transport_plan * qgraph_mask[:, :, :1].repeat(1,1,self.max_set_size), dim=1)

            # Adding 1 with lambda
            delete_factor_lambda = torch.ones_like(layout_transport_plan) * self.lambd + (1 - self.lambd) * layout_transport_plan

            # Add the zero scores of query for segment sum (indexing easiness)            
            inter_delete_factor = torch.stack((torch.ones_like(delete_factor_lambda), delete_factor_lambda), dim = 1).flatten()
            delete_factor = torch.cat(torch.split(inter_delete_factor, torch.stack((edge_counts, self.max_set_size - edge_counts), dim = 1).flatten().tolist())[0::2])
