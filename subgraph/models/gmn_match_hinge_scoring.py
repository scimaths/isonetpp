...
class GMN_match_hinge_scoring_injective_attention(torch.nn.Module):
    def __init__(self, av,config,input_dim):
        super(GMN_match_hinge_scoring_injective_attention, self).__init__()
        self.av = av
        self.device = 'cuda:0' if self.av.has_cuda and self.av.want_cuda else 'cpu'
        self.config = config
        self.input_dim = input_dim
    ...
    def build_layers(self):
        ...
        self.max_node_size = max(self.av.MAX_QUERY_SUBGRAPH_SIZE,self.av.MAX_CORPUS_SUBGRAPH_SIZE)
        self.cross_attention_module = CrossAttention(self.av, 'none', prop_config['node_state_dim'], self.max_node_size, injective_attention=True)
