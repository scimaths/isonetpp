...
class EarlyStoppingModule(object):
  ...
  def save_best_model(self, model, epoch): 
    save_dir = os.path.join(self.av.DIR_PATH, self.av.experiment_group + "/bestValidationModels")
    if not os.path.isdir(save_dir):
      os.makedirs(save_dir)
    name = self.av.TASK + "_" + self.av.DATASET_NAME + "_" + str(self.av.SEED) + "_" + self.av.time_key
