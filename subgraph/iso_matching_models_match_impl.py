...
def train(av,config):
  ...
  another_model = GMN_embed_maxsim_dot_corrected(av, copy.deepcopy(config), 1).to(device)
  ...
  while av.RUN_TILL_ES or run<av.NUM_RUNS:
    ...
    for i in range(n_batches):
      ...
      updated_state_dict["aggregator.MLP2.0.weight"] = another_model.state_dict()["aggregator.MLP2.0.weight"]
      updated_state_dict["aggregator.MLP1.0.bias"] = updated_state_dict["node_feature_processor.MLP.0.bias"]
      updated_state_dict["aggregator.MLP2.0.bias"] = another_model.state_dict()["aggregator.MLP2.0.bias"]
      updated_state_dict.pop("node_feature_processor.MLP.0.weight")
      updated_state_dict.pop("node_feature_processor.MLP.0.bias")
      another_model.load_state_dict(updated_state_dict)
      ...
      with torch.no_grad():
         prediction_another_model = another_model(batch_data, batch_data_sizes, batch_adj)
         if torch.sum((prediction - prediction_another_model) ** 2) > 1e-10:
